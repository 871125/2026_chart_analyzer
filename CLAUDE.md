...
- Backtest cost/filter flags: `--entry-mode {taker,maker}`, `--fee-rate`, `--maker-fee-rate`, `--slippage-pct`, `--min-stop-pct`, `--max-pending-candles`, `--max-same-direction`, `--intrabar {loss,skip}`
