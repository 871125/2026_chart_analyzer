...
> **⚠️ 중요 주의사항:** 이 봇은 바이낸스 선물 **양방향(Hedge) 모드**를 기준으로 작성되었습니다. 
> 봇을 실행하기 전에 반드시 바이낸스 선물(USD-M Futures) 거래 화면 설정(Preferences)에서 **Position Mode를 `Hedge Mode`로 변경**해 주세요. (One-way 모드에서는 API 주문 에러가 발생합니다.)

### 4.1. 주요 기능
- **데이터 수집:** 바이낸스(Binance) API를 사용하여 최신 OHLCV 시계열 데이터를 주기적으로 폴링합니다.
- **주문 실행:** 타점(EP) 도달 조건 만족 시, 바이낸스(Binance) 거래소 USD-M 무기한 선물 API를 활용해 **진입 주문(Market)과 동시에 익절(TP), 손절(SL) 주문을 세팅**합니다. (batchOrders 활용)
...
4. **안전한 진입 제어 및 중복 방지 (State Persistence)**
   - 진입가 도달 시 무지성으로 주문을 넣는 대신, **Binance API를 호출해 실제 거래소에 양방향(Hedge) 모드로 유지 중인 활성 포지션 개수를 확인**하여 설정된 `MAX_POSITIONS` 초과 여부를 철저히 검증합니다.
...
2. **환경변수 및 설정 입력**
   ...
   - `BINANCE_API_KEY`: 바이낸스 선물 API 키
   - `BINANCE_SECRET_KEY`: 바이낸스 선물 시크릿 키
...
- **[신규] 바이낸스(Binance) 마이그레이션 및 Hedge Mode 지원:** 빙엑스(BingX)에서 바이낸스 선물 API로 주문 모듈을 교체했습니다. 단일 주문에 TP/SL을 묶지 못하는 바이낸스의 특성을 반영하여 `batchOrders` 엔드포인트를 사용하도록 변경하였으며, 롱/숏 동시 진입/청산을 원활하게 관리하기 위해 **양방향(Hedge) 모드**를 도입했습니다.
