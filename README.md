...
- **주문 실행:** 타점(EP) 도달 조건 만족 시, 바이낸스(Binance) 선물 API의 `batchOrders` 기능을 활용해 **진입 주문(Market)과 동시에 익절(TP), 손절(SL) 주문을 하나의 요청으로 전송**합니다.
...
4. **안전한 진입 제어 및 중복 방지 (State Persistence)**
   - 진입가 도달 시 무지성으로 주문을 넣는 대신, **Binance API를 호출해 실제 거래소에 유지 중인 활성 포지션 개수를 확인**하여 설정된 `MAX_POSITIONS` 초과 여부를 철저히 검증합니다.
